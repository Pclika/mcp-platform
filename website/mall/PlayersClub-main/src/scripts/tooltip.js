...
class Tooltip {
  constructor(gridEl) {
    ...
    this.rowAnimationDirections = {
      product_name: { in: { yPercent: -100 }, out: { yPercent: -100 } },
      product_sku: { in: { yPercent: 100 }, out: { yPercent: 100 } },
      product_category: { in: { yPercent: 100 }, out: { yPercent: 100 } },
      product_price: { in: { yPercent: 100 }, out: { yPercent: 100 } },
  ...
  handleMouseMove = (e) => {
    ...
    this.scaleDownTimeout = setTimeout(() => {
      if (!this.hoverTarget) {
        ...
        this.updateTooltip({ product_name: '', product_sku: '', product_category: '', product_price: '' }, this.scaleDownTimeline, 'out');
  ...
  handleMouseEnter = (e) => {
    ...
    const productName = this.hoverTarget.dataset.productName || this.hoverTarget.dataset.stagename;
    const productSku = this.hoverTarget.dataset.productSku || this.hoverTarget.dataset.name;
    const productCategory = this.hoverTarget.dataset.productCategory || this.hoverTarget.dataset.genre;
    const productPrice = this.hoverTarget.dataset.productPrice || '';

    const updateTimeline = gsap.timeline();
    this.updateTooltip({ product_name: productName, product_sku: productSku, product_category: productCategory, product_price: productPrice }, updateTimeline, this.isTooltipVisible ? 'none' : 'in');
  };
  ...
  updateTextSlider(rowSelector, newValue, timeline, direction) {
    ...
    const animationDirection = this.rowAnimationDirections[rowField] || { in: { yPercent: 100 }, out: { yPercent: 100 } };
    ...
    if (direction === 'in') {
      ...
      const transitionOutDirection = {
        product_name: { yPercent: 100 },
        product_sku: { yPercent: -100 },
        product_category: { yPercent: -100 },
        product_price: { yPercent: -100 },
      }[rowField] || { yPercent: 0 };
